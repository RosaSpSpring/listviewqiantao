...
public class ReflexObjectUtil {
	 ...
	 */
	public static String getValueByKey(CurrentBean.DataBean mybean, String key) {
		// 得到类对象
		Class userCla = (Class) mybean.getClass();
		/* 得到类中的所有属性集合 */
		Field[] fs = userCla.getDeclaredFields();
		for (int i = 0; i < fs.length; i++) {
			Field f = fs[i];
			...
			try {
				if (f.getName().endsWith(key)) {
					System.out.println("单个对象的某个键的值==反射==" + f.get(mybean));
					return (String) f.get(mybean);
				}
			} catch (IllegalArgumentException e) {
				e.printStackTrace();
			} catch (IllegalAccessException e) {
